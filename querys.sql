...
-- default Access Path is Seq Scan
explain select s.sname , s.age from sailors s;

-- 2.
select distinct s.sname , s.age from sailors s;

-- default access path is Seq Scan on key(sname , age)
explain select distinct s.sname , s.age from sailors s;

-- 3.
select * from sailors s where s.age > 7;

-- default access path is Seq Scan on table s
explain select * from sailors s where s.age > 7;
